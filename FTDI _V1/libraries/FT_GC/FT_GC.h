...
typedef enum FT_Status {
	/* Common enums */
	FT_OK = 0, FT_ERROR = 1, FT_WARNING = 2, FT_ERROR_INIT = 3, FT_ERROR_CHIPID = 4,
...
typedef enum FT_AEStatus {
	FT_AE_OK = 0, FT_AE_ERROR_FORMAT = 1, FT_AE_ERROR_SAMPLINGFREQ_OUTOFRANGE = 2,	//assert for boundary
...
typedef enum FT_GEStatus {
	FT_GE_OK = 0, FT_GE_BUSY = 1, FT_GE_FINISHED = 2,

	/* Graphics related error enums */
	FT_GE_ERROR_INVALID_PRIMITIVE = 20, FT_GE_ERROR_INVALID_BITMAP_FORMAT = 21, FT_GE_ERROR_INVALID_BITMAP_HANDLENUM = 22, FT_GE_ERROR_VERTEX_OUTOFRANGE = 23,
...
typedef enum FT_TEStatus {
	FT_TE_OK = 0, FT_TE_ERROR_RZTHRESHOLD = 1,		//threshold out of bound
...
public:
	...
	FT_Status Init(uint16_t hperiod, uint16_t vperiod, uint16_t hfrontporch, uint16_t hbackporch, uint16_t hpulsewidth, uint16_t vfrontporch, uint16_t vbackporch, uint16_t vpulsewidth,
	...
	FT_GEStatus PrintText(int16_t x, int16_t y, uint8_t Font, uint16_t Options, const char *fmt, ...);FT_GEStatus PrintTextFlash(int16_t x, int16_t y,
			uint8_t Font, uint16_t Options, const char *fmt PROGMEM, ...);
	...
	/* Global variables */
	uint8_t GInit, TrnsFlag;	//Global flag to indicate that initialization is done
...
template<class FT_Trans>
FT_Status FT_GC<FT_Trans>::Init(uint8_t ResType, uint16_t options1 = 0, bool flipped = 0) {
	if (IntPin != -1) {
...
template<class FT_Trans>
FT_Status FT_GC<FT_Trans>::Init(uint16_t hperiod, uint16_t vperiod, uint16_t hfrontporch, uint16_t hbackporch, uint16_t hpulsewidth, uint16_t vfrontporch, uint16_t vbackporch, uint16_t vpulsewidth,
...
FT_GEStatus FT_GC<FT_Trans>::PrintTextFlash(int16_t x, int16_t y, uint8_t Font,
		uint16_t Options, const char *fmt PROGMEM, ...) {
