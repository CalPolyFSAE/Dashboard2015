...
void DashLCD::updateDisplay() {
    ...
    { // False for rotary disable//(DashboardData.rotaryOverride) {
        rotaryOverride ();
    }
...
void DashLCD::CAN_RX_Int( CPFECANLib::MSG *msg, uint8_t mobNum ) {
    ...
    switch (msg->identifier.standard) {
        ...
        case WarningCANMessageID:
            memcpy ((void *) &warningCANData, msg->data,
                    sizeof(warningCANData));
...
void DashLCD::RX_DashCAN1( bool interruptMode ) {
    CPFECANLib::enableMOBAsRX_PROGMEM (DashCAN1Mob, &DashCAN1MSG, &DashCAN1Mask,
                                       interruptMode);
}

void DashLCD::RX_DashCAN2( bool interruptMode ) {
    CPFECANLib::enableMOBAsRX_PROGMEM (DashCAN2Mob, &DashCAN2MSG, &DashCAN2Mask,
                                       interruptMode);
}

void DashLCD::RX_DashCAN3( bool interruptMode ) {
    CPFECANLib::enableMOBAsRX_PROGMEM (DashCAN3Mob, &DashCAN3MSG, &DashCAN3Mask,
                                       interruptMode);
}

void DashLCD::RX_DashCAN4( bool interruptMode ) {
    CPFECANLib::enableMOBAsRX_PROGMEM (DashCAN4Mob, &DashCAN4MSG, &DashCAN4Mask,
                                       interruptMode);
}
...
//display waiting for can data message to screen
...
void DashLCD::rotaryOverride() {
    ...
    LCD.ColorRGB (~color);
    LCD.PrintText (FT_DISPLAYWIDTH / 2, FT_DISPLAYHEIGHT / 2, 31, FT_OPT_CENTER,
                   "%d", position);
